...
| **Simulation (what-if rollout)** | **TGPT: predict next transaction's fields, roll forward** | **`run_gen.py`: gap-band/amount-band/rail heads on h_USR + `simulate()` through the frozen encoder** | **C8 pre-registered; smoke 2/3 fields beat naives; full = runbook run H** |
