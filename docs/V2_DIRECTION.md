...
| **C11** entity-level fusion (`run_c11.py`, `data/lifelong.py`): in-flight cancel/return heads read [prefix ⊕ h_entity], where h_entity encodes the account's FULL message timeline across PRIOR payments (fractional-day dt — the C13 resolution item lives here) | PRAGMA's core lesson (everything the bank knows behind every prediction) at our in-flight grain: does account history lift current-payment recall/return risk? Generator knob `recall_momentum` (default OFF — corpora byte-reproducible) plants account-level recall clustering so the signal exists to find | momentum-ON fleet: fused beats base by >= +5pp PR-AUC on cancel or return, held-out accounts; momentum-OFF fleet is the NULL CONTROL and must show no comparable lift | **measured (CPU, 500x12k): FAIL — and the interesting kind.** The null control HELD (momentum-OFF fused delta −0.5pp) but the ON fleet showed no lift either (+0.1pp) despite the planted clustering (verified present by test). Diagnosis: h_entity is pretrained to reconstruct Ccy/msg_type — rare camt.056/pacs.004 legs plausibly do not survive into the pooled history vector, so the fusion carries the timeline's shape but not its recall content. Follow-up options (unregistered): recon targets including tx_sts/recall legs, or explicit count features (n_prior_recalls) alongside h_entity — the C12 momentum-ON reading will partly arbitrate (its ll_had_recall feature is exactly that count). Serving fusion stays deferred — nothing measured earns it yet |
| **C12** life-long milestones (`data/lifelong.py`, `run_c12.py`): prior-only timed firsts (tenure, first cross-border, first recall, prior count) fused into intake probes [f(x) ⊕ lifelong] | PRAGMA's life-long events at our intake grain, aimed at the documented cold-start weak spot (young-account slice) | young-slice accuracy lift >= +1pp (rail/risk mean) without full-slice damage; pre-stated: the DEFAULT generator plants no tenure signal, so a null is the expected honest outcome there — the informative run is momentum-ON where first_recall predicts returns | **measured (CPU, 4000x20k): NULL on both readings — with a measurement-design finding.** Default generator: deltas ~0, the pre-stated null. Momentum-ON: ALSO null (return 0.016 -> 0.016), but 98.7% of eval is "young" — at ~5 payments/account, prior-recall milestones barely exist, so the INTAKE FLEET cannot carry entity signals (the velocity finding, rediscovered at intake). Neither reading arbitrates the C11 diagnosis; the dense-fleet momentum reading (500x12k, same fleet velocity needed) is the clean arbitration and is pending. Cold-start measurement needs a fleet with real tenure spread — recorded as a generator gap |
