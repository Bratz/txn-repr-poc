...
| **C8** the TransactionGPT tier (`run_gen.py`): next-field heads on frozen h_USR (gap band / amount band / rail) + generative rollout through the frozen encoder | whether the twin can PREDICT the next transaction's fields better than per-actor naives (band of median gap, band of median amount, majority rail) — the simulation tier of the three-paper merge (docs/PAYMENT_TWIN_MERGE.md) | held-out-actor field accuracy beats the naive on >= 2 of 3 fields at full config | **measured (CPU, full 500x12k cadence fleet): FAIL — 1/3.** gap 0.359 vs 0.315 (WIN), amount 0.333 vs 0.577 (loss), rail 0.470 vs 0.496 (loss — smoke's rail win did not survive scale). Consistent with run_next: "when" is the one field the learned representation earns; amount and rail belong to per-actor naives on a cadence fleet (fixed schedules make the naives strong BY DESIGN — the run_gen docstring pre-stated this). Consequence: the rollout machinery stands as tested mechanism, but NO /simulate serving integration — the measure-first gate holds. Revisit on real data where cadences are not generator-perfect |
| **C9** richer adapter family on the SAME frozen encoder: LoRA-family low-rank adapters (base weights frozen — the invariant holds; LoRA never fine-tunes the base) vs the linear probes and vs CatBoost, on the C2 task suite, same splits | whether C2's 44.5 pp gap to CatBoost was the price of the *adapter family* (linear probes / prompt-family) rather than the freeze itself — the no-conflict reconciliation of Raman's parameter-efficiency with PRAGMA's measured LoRA gains | closes >= half the probe-to-CatBoost gap on the C2 risk task at matched trainable-parameter honesty (report adapter param count) → adopt LoRA as the capacity-escalation tier for new questions; a miss localises the bottleneck at representation bandwidth (→ the C10 question) | **implemented + measured locally (CPU, 20k rows, converged)**: probe 0.152 / LoRA **0.301** / CatBoost 0.517 → gap closure **40.7% — FAIL** vs the 50% bar (early-stopped epoch 13, best-val restored; FFN-only targets — torch's fused MHA cannot be wrapped, documented). LoRA DOUBLES the probe with the base frozen, but ~60% of the gap survives → the residual is representation bandwidth, pre-registered as C10. Full-scale verdict (1M rows, GPU) = runbook run I |
| **C10** read-out bandwidth: a head over the encoder's FULL token outputs (all column tokens, mean-pool and concat variants) vs the single-token f(x) probe — same frozen base, same splits, same task | whether C2's residual deficit after C9 is the single-token read-out contract (the row compressed to one 512-d vector before any head sees it — TGPT's bandwidth diagnosis, their virtual-token motivation) rather than encoder capacity | closes >= half of the gap that C9's LoRA arm left open (LoRA→CatBoost), at reported head-parameter counts | **measured (CPU, 20k, matched to C9): FAIL** — single 0.152 / mean 0.163 / **concat 0.261** / LoRA ref 0.301 / CatBoost 0.517. Bandwidth alone (+72% over the single token, zero backbone training) is real but lands BELOW converged LoRA: closure of the LoRA-left gap is negative. Read with C9: neither adapter capacity (40.7% closure) nor read-out bandwidth alone reaches the tree on a rule-generated label — C2's original routing verdict (ship the tree for complete-row rule labels) stands CONFIRMED by two independent interventions. (The optional LoRA x wide read-out interaction test remains unregistered — the twin routes this regime to trees) |
| **C11** entity-level fusion (`run_c11.py`, `data/lifelong.py`): in-flight cancel/return heads read [prefix ⊕ h_entity], where h_entity encodes the account's FULL message timeline across PRIOR payments (fractional-day dt — the C13 resolution item lives here) | PRAGMA's core lesson (everything the bank knows behind every prediction) at our in-flight grain: does account history lift current-payment recall/return risk? Generator knob `recall_momentum` (default OFF — corpora byte-reproducible) plants account-level recall clustering so the signal exists to find | momentum-ON fleet: fused beats base by >= +5pp PR-AUC on cancel or return, held-out accounts; momentum-OFF fleet is the NULL CONTROL and must show no comparable lift | **measured (CPU, 500x12k): FAIL, null control held — and CLOSED with the correct explanation.** Initial diagnosis ("h_entity's recon objective swallows rare recall legs") is RETRACTED as primary: a direct check on the dense momentum fleet shows the planted signal is real per-row (P(cancel given prior recall) = 2.04x baseline) but fires on only 5.2% of payments — the raw feature's ceiling is AP 0.036 vs prevalence 0.033, i.e. **+0.3pp maximum extractable**, against a +5pp threshold. The experiment was unpassable as parameterized; no model missed anything. Lever identified: milestone COVERAGE (longer histories / higher base recall rate / stronger momentum), not architecture. Lesson logged: pre-registrations now require a power check (planted effect x coverage -> max metric delta) before runs are burned. Serving fusion stays deferred |
| **C12** life-long milestones (`data/lifelong.py`, `run_c12.py`): prior-only timed firsts (tenure, first cross-border, first recall, prior count) fused into intake probes [f(x) ⊕ lifelong] | PRAGMA's life-long events at our intake grain, aimed at the documented cold-start weak spot (young-account slice) | young-slice accuracy lift >= +1pp (rail/risk mean) without full-slice damage; pre-stated: the DEFAULT generator plants no tenure signal, so a null is the expected honest outcome there — the informative run is momentum-ON where first_recall predicts returns | **measured (CPU): NULL on all three readings, CLOSED.** Default 4000x20k: pre-stated null. Momentum-ON 4000x20k: null, 98.7% of eval "young" (~5 payments/account — the intake fleet cannot carry entity signals; the velocity finding rediscovered). Momentum-ON dense 500x12k: null, and the direct signal check explains all of it — the milestone feature's coverage (5.2%) caps its contribution at ~+0.3pp, an order of magnitude under the threshold (see C11). The feature path is correct and tested; the corpus cannot reward it as parameterized. Cold-start measurement needs a fleet with real tenure spread — generator gap stands |
