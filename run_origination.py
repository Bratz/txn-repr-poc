"""
Multi-source fusion measurement: does the pain.001 ORIGINATION CONTEXT (channel/device/
session detail captured at acquisition) add signal the interbank ISO view cannot carry?
...
Three probes on the same held-out outward payments, label = atoFlag (account-takeover drain):
  ...
  context only  linear probe on the origination-context features
  FUSED         probe on [f(x) (+) context features]

By construction the ATO pattern (new device + credential change + payee-add + burst login)
lives only in the origination context, so the honest expectation is ISO ~ prevalence and
fusion ~ context - the measurement demonstrates that the origination slice is NECESSARY for
this label, not that fusion is magic. Because the context exists at t=0 (pain.001 birth), the
same fusion is legitimate for EVERY initiation-time head (STP/ETA/cancel/return).

  python run_origination.py --smoke
"""
...
from data.pain001_context import build_origination_context, ctx_matrix
...
def main():
    ap = argparse.ArgumentParser(description="pain.001 origination context - multi-source fusion eval")
    ...
    ap.add_argument("--out", default=str(ROOT / "results_origination.json"))
    args = ap.parse_args()
    ...
    ctx, events = build_origination_context(pay)
    print(f"payments {len(pay):,} -> outward with origination context {len(ctx):,} "
    ...
    res = {
        ...
        "context_only_pr_auc": _pr(C[tr], y[tr], C[ev], y[ev]),
    ...
    print(f"  origination ctx    PR-AUC {res['context_only_pr_auc']:.3f}")
